...
class BotValidator {
    ...
    try {
      ...
      if (labelResult.labels_added && labelResult.labels_added.length > 0) {
        this.log(
          ...
          'Auto-labeling',
          SUCCESS,
          `Applied ${labelResult.labels_added.length} labels`
        );
      } else {
      ...
      if (failureResult.created) {
        this.log(
          ...
          SUCCESS,
          'Investigation issue created for failure'
        );
      } else {
        this.log(
          'githubApp',
          'Failure Investigation',
          SUCCESS,
          'Workflow failure handled appropriately'
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (analysis && analysis.repository === 'xcloud-bot') {
        this.log(
          ...
          'Repository Analysis',
          SUCCESS,
          `Health: ${analysis.overall_health}, Success Rate: ${analysis.success_rate}`
        );
      } else {
      ...
      if (allAnalysis && allAnalysis.total_repositories === 5) {
        this.log(
          'workflows',
          'All Repositories Analysis',
          SUCCESS,
          `Analyzed ${allAnalysis.total_repositories} repositories`
        );
      } else {
        this.log(
          'workflows',
          'All Repositories Analysis',
          FAILURE,
          'Failed to analyze all repositories'
        );
      }
      ...
      if (createdCount === workflowTypes.length) {
        this.log(
          ...
          'Workflow Creation',
          SUCCESS,
          `Created ${createdCount} workflow types`
        );
      } else {
        this.log(
          'workflows',
          'Workflow Creation',
          FAILURE,
          `Only created ${createdCount}/${workflowTypes.length} workflow types`
        );
      }
  ...
  generateReport() {
    ...
    for (const category of categories) {
      ...
      console.log(
        `  📊 Success Rate: ${tests.length > 0 ? Math.round((passed / tests.length) * 100) : 0}%`
      );
    }
    ...
    console.log(
      `  Overall Success Rate: ${totalTests > 0 ? Math.round((totalPassed / totalTests) * 100) : 0}%`
    );
